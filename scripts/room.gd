...
func _close_entrance() -> void:
	for entry_position in entrance.get_children():
		...
		tilemap.set_cell(0, entry_position.position / (4.0 * 8.0), 3, Vector2i(1, 0))
		tilemap.set_cell(0, (entry_position.position / (4.0 * 8.0)) + Vector2.LEFT, 3, Vector2i(1, 5))
		tilemap.set_cell(0, (entry_position.position / (4.0 * 8.0)) + Vector2.RIGHT, 3, Vector2i(0, 5))
